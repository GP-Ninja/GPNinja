...
Future<List<ClicknoteSectionStruct>> moveOptionBackward(
  ...
  if (subjectOptionIndex <= 0 || subjectOptionIndex >= options.length) {
    throw Exception(
        "Option index out of range or already at the start of the list");
  }

  // Swap the option with the previous option
  var currentOption = options[subjectOptionIndex];
  var previousOption = options[subjectOptionIndex - 1];

  // Update the option_index for both options
  currentOption.optionIndex = subjectOptionIndex - 1;
  previousOption.optionIndex = subjectOptionIndex;

  // Swap the options in the list
  options[subjectOptionIndex] = previousOption;
